...
*[→ full entry](../claims/gaussian-process/C1.md)*
...
*[→ full entry](../claims/gaussian-process/C2.md)*
...
*[→ full entry](../claims/gaussian-process/C3.md)*
...
*[→ full entry](../claims/gaussian-process/C4.md)*
...
*[→ full entry](../claims/gaussian-process/C5.md)*
...
**✓ HP1** · for an **EQ / Matérn GP length scale** where the design is length-scale-sensitive → an *[→ entry](../recs/gaussian-process/HP1.md)*
...
**✗ HP2** · for an **EQ length scale** → a **half-normal / light-tailed** prior does **NOT** work. *[→ entry](../recs/gaussian-process/HP2.md)*
...
**✓ HP3** · for a **Matérn length scale in d≤3** → the **PC prior = Frechet(d/2)** (exactly inverse-gamma *[→ entry](../recs/gaussian-process/HP3.md)*
in 2D) works.
...
**✗ HP4** · for **EQ GP hyperparameters** → **maximum marginal likelihood (MML) without a prior** does *[→ entry](../recs/gaussian-process/HP4.md)*
**NOT** work.
...
**✗ HP5** · for **GP (σ, ℓ)** in a fixed-domain low-n setting → a **reference prior** does **NOT** work *[→ entry](../recs/gaussian-process/HP5.md)*
well.
...
**✗ HP6** · for **GP hyperparameter identifiability** → reading only the **marginal posteriors of ℓ and *[→ entry](../recs/gaussian-process/HP6.md)*
σ** does **NOT** work.
...
**✓ HP7** · for **GP hyperparameter identifiability** → inspecting the **2-D joint posterior of (ℓ, σ)** *[→ entry](../recs/gaussian-process/HP7.md)*
works.
...
**✗ HP8** · for a **GP with ARD** over several covariates → using the **ARD inverse-length-scales for *[→ entry](../recs/gaussian-process/HP8.md)*
...
**✓ HP9** · for a **designed study** whose sampling was motivated by length-scale knowledge → **setting *[→ entry](../recs/gaussian-process/HP9.md)*
...
**✗ HP10** · for an **observational (non-designed) GP** → **retrofitting the ρ prior to the data span** *[→ entry](../recs/gaussian-process/HP10.md)*
does **NOT** work.
...
**✗ HP11** · for a **GP under cross-validation or sequential data collection** → the **naive *[→ entry](../recs/gaussian-process/HP11.md)*
...
**✗ HP12** · for a **Matérn GP** → **inferring the smoothness ν jointly** does **NOT** work. *[→ entry](../recs/gaussian-process/HP12.md)*
...
**✗ HP13** · for a **non-centered spline/GP term** flagged by a **power-scaling prior-sensitivity *[→ entry](../recs/gaussian-process/HP13.md)*
...
**✓/✗ HP14** · for an **R2D2 prior on a GP** → substituting a **pseudo-variance independent of model *[→ entry](../recs/gaussian-process/HP14.md)*
...
**✓ G1** · for a **latent-GP field** with fixed hyperparameters and a relatively **diffuse** likelihood *[→ entry](../recs/gaussian-process/G1.md)*
...
**✗ G2** · for a **high-dimensional GP prior** → the **centered parameterization** does **NOT** work — *[→ entry](../recs/gaussian-process/G2.md)*
and fails *invisibly*.
...
**✓ G3** · for a **latent-Gaussian model** y = Au + ε, u ~ N(0,Q(θ)⁻¹) → **marginalize u and run MCMC *[→ entry](../recs/gaussian-process/G3.md)*
...
**✓/✗ G4** · for a **GP regression** y ~ N(a+f, σ) with **divergences concentrated at small σ** → *[→ entry](../recs/gaussian-process/G4.md)*
...
**✓ G5** · for an **NCP GP with a Gaussian likelihood linear in the latent** → a **Laplace-marginalizing *[→ entry](../recs/gaussian-process/G5.md)*
...
**✗ G6** · for a **Laplace marginal over a high-dimensional latent** → the **default L-BFGS *[→ entry](../recs/gaussian-process/G6.md)*
...
**✓ G7** · for that **high-dimensional Laplace inner solve** → raising **L-BFGS maxiter to ~300–500** *[→ entry](../recs/gaussian-process/G7.md)*
works.
...
**✗ G8** · for **frozen/flat GP traces** with a collapsed step size → reading them as **successful *[→ entry](../recs/gaussian-process/G8.md)*
...
**✓ W1** · for **multi-chain HMC/NUTS with windowed warmup** → **adaptive warmup driven by *[→ entry](../recs/gaussian-process/W1.md)*
...
**✗ W2** · for a **chain that was never converging** → **warmup** does **NOT** rescue it. *[→ entry](../recs/gaussian-process/W2.md)*
...
**✗ W3** · for a model with **structural non-identifiability between chains** (factor indeterminacy, *[→ entry](../recs/gaussian-process/W3.md)*
...
**✗ S1** · for an **exact dense GP with N more than a few hundred** → **full-covariance HMC/NUTS** does *[→ entry](../recs/gaussian-process/S1.md)*
**NOT** scale.
...
**✓ S2** · for a **large smooth stationary GP** → a **Hilbert-space basis-function (HSGP) low-rank *[→ entry](../recs/gaussian-process/S2.md)*
approximation** works.
...
**✓ S3** · for **low-dimensional (1-D, time-like) ordered inputs with a Matérn-class kernel** → a *[→ entry](../recs/gaussian-process/S3.md)*
...
**✓ S4** · for a **sparse-precision latent-Gaussian model** → **parameterizing via the precision Q(θ), *[→ entry](../recs/gaussian-process/S4.md)*
...
**✗ S5** · for a **predictive-process / global-inducing-point GP with a dense design matrix** → *[→ entry](../recs/gaussian-process/S5.md)*
...
**✓ S6** · for a **sparse SPD precision Q with L already computed** → the **Takahashi recursions** to *[→ entry](../recs/gaussian-process/S6.md)*
...
**✗ S7** · for **gradient-based hyperparameter inference through the partial inverse** → relying on it *[→ entry](../recs/gaussian-process/S7.md)*
...
**✗ S8** · for **sparse Cholesky in JAX** → relying on **dynamically-shaped output** does **NOT** work. *[→ entry](../recs/gaussian-process/S8.md)*
...
**✓ M1** · for a **stationary GP** → a **sparse-precision (SPDE/FEM) representation** works **iff** the *[→ entry](../recs/gaussian-process/M1.md)*
...
**✗ M2** · for the **squared-exponential (RBF) kernel** → an **SPDE/FEM sparse representation** does *[→ entry](../recs/gaussian-process/M2.md)*
...
**✓ M3** · for a **Markovian GP** → building a **sparse precision Q via FEM** works only with **both** *[→ entry](../recs/gaussian-process/M3.md)*
...
**✓ M4** · for **GPs on non-Euclidean domains** (e.g. the sphere) → the **RKHS/SPDE characterization *[→ entry](../recs/gaussian-process/M4.md)*
...
**✗ P1** · for an **exact/latent GP predicting at new inputs X\*** → the **generic parametric predictive *[→ entry](../recs/gaussian-process/P1.md)*
...
**✓ P2** · for **GP out-of-sample prediction** → explicitly **conditioning on the training data via the *[→ entry](../recs/gaussian-process/P2.md)*
...
**✗ A1** · for an **additive / multi-component signal decomposition** (multi-scale GPs, shared global + *[→ entry](../recs/gaussian-process/A1.md)*
...
**✓ A2** · for **additive GP components** → **priors that concentrate each component around a unique, *[→ entry](../recs/gaussian-process/A2.md)*
...
**✗ SP1** · for a **Bayesian penalized-spline / GAM smooth** → reasoning about global behavior from the *[→ entry](../recs/gaussian-process/SP1.md)*
...
**✓ SP2** · for a **monotone/shape-constrained smooth** wanted from a linear spline basis → imposing the *[→ entry](../recs/gaussian-process/SP2.md)*
...
**✓ AP1** · for a **grouped / repeated-measures contrast** (A/B test with per-day counts and revenue) → *[→ entry](../recs/gaussian-process/AP1.md)*
...
**✗ AP2** · for the **same contrast when between-group variation is genuinely unstructured noise** → a *[→ entry](../recs/gaussian-process/AP2.md)*
...
**✓ AP3** · for **population-mean estimation under covariate-dependent randomization** (Robins–Ritov) → *[→ entry](../recs/gaussian-process/AP3.md)*
...
**✓ E1** · for a **GP funnel with divergences** → **pairs-plotting the ENSEMBLE of divergent draws** *[→ entry](../recs/gaussian-process/E1.md)*
...
**✓ E2** · for **locating *where* a divergence occurred** → reporting the trajectory **endpoint weighted *[→ entry](../recs/gaussian-process/E2.md)*
...
**✗ E3** · for a **geometric GP pathology** → cranking **adapt_delta / target_accept to an extreme** to *[→ entry](../recs/gaussian-process/E3.md)*
...
**✗ E4** · for **prior-improbable posteriors on GP terms** → reading them as a **model problem** without *[→ entry](../recs/gaussian-process/E4.md)*
...
**✗ EB1** · for **setting a GP prior scale from a pre-fit to the same data** → **empirical-Bayes prior *[→ entry](../recs/gaussian-process/EB1.md)*
...
**✓ EB2** · for **checking a GP/spline prior** → running a **prior-predictive check against implicit *[→ entry](../recs/gaussian-process/EB2.md)*
...
**✓ PB1** · for an **intended bimodal / bounded prior** (e.g. Beta) that misbehaves under HMC → swapping *[→ entry](../recs/gaussian-process/PB1.md)*

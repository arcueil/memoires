...
- follow-up & related discussion (in the corpus): the less-conservative target **was** implemented, tested, and reverted — mc-stan:9532 (betanalpha, "Request for Volunteers to Test Adaptation Tweak"; the thread ends with the maintainers reverting stan#2836). An **alternative** gradient-based metric + step-size adaptation (Adrian Seyboldt / covadapt, as used in nuts-rs / nutpie) is discussed as significantly faster — mc-stan:32259. Background: what the acceptance statistic `accept_stat__` is vs `adapt_delta` — mc-stan:9931; dual-averaging step-size restart quirks — mc-stan:5995.

...
*[→ full entry](../claims/CC-convergence-diagnostics/C1.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C2.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C3.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C4.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C5.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C6.md)*
...
*[→ full entry](../claims/CC-convergence-diagnostics/C7.md)*
...
**✗ A1** · when a chain is stationary/irreducible/aperiodic and you take that as certifying a valid *[→ entry](../recs/CC-convergence-diagnostics/A1.md)*
...
**✗ A2** · when a heavy-tailed / non-geometrically-ergodic target passes split-R̂ and k̂ at short N *[→ entry](../recs/CC-convergence-diagnostics/A2.md)*
...
**✓ A3** · when you need to *trust* a finite-N CLT error bar → require **preasymptotic** (not merely *[→ entry](../recs/CC-convergence-diagnostics/A3.md)*
...
**✗ A4** · when the expectand has undefined mean or infinite variance → a Monte-Carlo point *[→ entry](../recs/CC-convergence-diagnostics/A4.md)*
...
**✓ A5** · when you discard warmup states → valid **only if** the full chain (including warmup) would *[→ entry](../recs/CC-convergence-diagnostics/A5.md)*
...
**✗ A6** · when a chain is stationary but explores the typical set inefficiently → relying on *[→ entry](../recs/CC-convergence-diagnostics/A6.md)*
...
**✗ B1** · when all chains start in the same basin (or all become identically pathological) → R̂ does *[→ entry](../recs/CC-convergence-diagnostics/B1.md)*
...
**✗ B2** · when you run a single chain on a multimodal/mixture target → per-chain diagnostics do *[→ entry](../recs/CC-convergence-diagnostics/B2.md)*
...
**✓ B3** · when you suspect metastability / label switching → run multiple chains from overdispersed *[→ entry](../recs/CC-convergence-diagnostics/B3.md)*
...
**✓ B4** · when detecting metastability under a compute budget → prefer more **overdispersed** chains *[→ entry](../recs/CC-convergence-diagnostics/B4.md)*
...
**✗ B5** · when a reducibility barrier confines all chains to one partition cell → split-R̂ and k̂ are *[→ entry](../recs/CC-convergence-diagnostics/B5.md)*
...
**✗ B6** · when you "discard the bad chains and keep the good ones" (burn-in framing) → does **NOT** *[→ entry](../recs/CC-convergence-diagnostics/B6.md)*
work.
...
**✗ B7** · when divergences / E-BFMI / tree-depth are clean on a well-separated mixture → concluding *[→ entry](../recs/CC-convergence-diagnostics/B7.md)*
...
**✗ C1** · when you quote one number as "the chain's ESS" → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/C1.md)*
...
**✗ C2** · when the chain is highly autocorrelated (λ̂=ESŜ/N < 1/10000) → trusting the ESS estimate *[→ entry](../recs/CC-convergence-diagnostics/C2.md)*
does **NOT** work.
...
**✗ C3** · when fewer than 0.001 effective samples per transition → trusting reported n_eff does *[→ entry](../recs/CC-convergence-diagnostics/C3.md)*
**NOT** work.
...
**✓/✗ C4** · when negative autocorrelations (antithetic mixing) appear → ESS>N is *real* *[→ entry](../recs/CC-convergence-diagnostics/C4.md)*
...
**✗ C5** · when RWM ESS/iteration decays with dimension → tuning the proposal scale to fix it does *[→ entry](../recs/CC-convergence-diagnostics/C5.md)*
**NOT** work.
...
**✗ C6** · when a self-consistency std_ratio is formed after collapsing the reference array across *[→ entry](../recs/CC-convergence-diagnostics/C6.md)*
...
**✓ C7** · when ESS/n_eff is untrustworthy in the struggling regime → **refit small purely for *[→ entry](../recs/CC-convergence-diagnostics/C7.md)*
...
**✗ D1** · when you thin a well-mixing chain to "improve" ESS → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/D1.md)*
...
**✓ D2** · when memory is the binding constraint on a necessarily-long chain → thinning **for storage *[→ entry](../recs/CC-convergence-diagnostics/D2.md)*
reduction** works.
...
**✗ D3** · when you thin a periodic chain by a stride that is a multiple of its period M → does *[→ entry](../recs/CC-convergence-diagnostics/D3.md)*
...
**✓ D4** · when thinning is genuinely needed → use **deterministic post-run thinning by a stride COPRIME with the period** works. *[→ entry](../recs/CC-convergence-diagnostics/D4.md)*
...
**✓ E1** · when split-R̂ passes but you need to know the estimate is trustworthy → also compute **k̂** *[→ entry](../recs/CC-convergence-diagnostics/E1.md)*
...
**✗ E2** · when you read k̂ as a generic "sampler-quality" flag → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/E2.md)*
...
**✓ E3** · when a heavy-tailed target makes the identity function fail k̂ → estimate **bounded *[→ entry](../recs/CC-convergence-diagnostics/E3.md)*
...
**✗ E4** · when you want a probability *density* from samples via KDE → does **NOT** work as *[→ entry](../recs/CC-convergence-diagnostics/E4.md)*
...
**✗ E5** · when estimating a small tail probability p by plain Monte Carlo → assuming a modest sample *[→ entry](../recs/CC-convergence-diagnostics/E5.md)*
...
**✗ F1** · when NaN R̂/n_eff fires on a stored constant (fixed boundary state) → reading it as a *[→ entry](../recs/CC-convergence-diagnostics/F1.md)*
...
**✗ F2** · when NaN R̂/n_eff appears for S(0)=1 in a survival display grid → treating it as *[→ entry](../recs/CC-convergence-diagnostics/F2.md)*
...
**✗ F3** · when a top-line "largest R-hat is NA, chains have not mixed" / "Bulk/Tail ESS too low" *[→ entry](../recs/CC-convergence-diagnostics/F3.md)*
...
**✗ F4** · when a "multiplier is 0, but must be > 0!" (scale-underflow) exception appears **only** in *[→ entry](../recs/CC-convergence-diagnostics/F4.md)*
...
**✗ F5** · when a small conjugate Beta-Bernoulli gives a different posterior mean every re-run → *[→ entry](../recs/CC-convergence-diagnostics/F5.md)*
...
**✗ F6** · when a subset of parameters shows R̂>1.1 under a two-component (inner_τ≪outer_τ) mixture *[→ entry](../recs/CC-convergence-diagnostics/F6.md)*
...
**✓ G1** · when running multiple chains with windowed warmup → feed **between-chain R̂ / bulk-ESS *[→ entry](../recs/CC-convergence-diagnostics/G1.md)*
...
**✗ G2** · when you halt sampling the instant ESS first crosses a threshold → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/G2.md)*
...
**✓ H1** · when the acceptance probability has a small *bounded relative error* (float32 on a *[→ entry](../recs/CC-convergence-diagnostics/H1.md)*
...
**✗ H2** · when you treat every "wrong acceptance probability" the same → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/H2.md)*
...
**✓ H3** · when bounding the error of an approximate MH chain → decompose it into **J correlated exact *[→ entry](../recs/CC-convergence-diagnostics/H3.md)*
...
**✗ I1** · when you rely on absolute continuity (support containment) for IS validity → does **NOT** *[→ entry](../recs/CC-convergence-diagnostics/I1.md)*
work.
...
**✗ I2** · when you check only that the IS ratio has finite variance → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/I2.md)*
...
**✓ I3** · when using IS → deploy it to **refine an already-good approximation** (e.g. post-VI), not *[→ entry](../recs/CC-convergence-diagnostics/I3.md)*
...
**✗ J1** · when you read an oscillating / spiking ELBO curve as a bug or a bad variational family → *[→ entry](../recs/CC-convergence-diagnostics/J1.md)*
does **NOT** work.
...
**✗ J2** · when you take ELBO-convergence as *mean*-convergence for MFVI on an ill-conditioned *[→ entry](../recs/CC-convergence-diagnostics/J2.md)*
...
**✓ J3** · when unsure whether to declare dependence (to_event) vs conditional independence (plate) in *[→ entry](../recs/CC-convergence-diagnostics/J3.md)*
...
**✓ K1** · when validating an MCMC recipe against ground truth → a conservative **three-band max-z *[→ entry](../recs/CC-convergence-diagnostics/K1.md)*
gate** works.
...
**✓ K2** · when validating **iid VI draws** (n≪GT_ESS) against a high-ESS ground truth → use a *[→ entry](../recs/CC-convergence-diagnostics/K2.md)*
...
**✗ K3** · when you apply the standard **z<2.0** gate to iid VI draws → does **NOT** work. *[→ entry](../recs/CC-convergence-diagnostics/K3.md)*

...
*[→ full entry](../claims/spatial-areal/C1.md)*
...
*[→ full entry](../claims/spatial-areal/C2.md)*
...
*[→ full entry](../claims/spatial-areal/C4.md)*
...
*[→ full entry](../claims/spatial-areal/C5.md)*
...
**✓ S1** · for a latent-Gaussian spatial model y = Au + ε, u ~ N(0, Q(θ)⁻¹) with a **sparse structured *[→ entry](../recs/spatial-areal/S1.md)*
...
**✗ S2** · for the same model when the **design matrix Z is dense** (subset-of-regressors / *[→ entry](../recs/spatial-areal/S2.md)*
...
**✗ T1** · for a **CAR/GMRF model parameterized through precision Q** → interpreting the scalar τ as *[→ entry](../recs/spatial-areal/T1.md)*
...
**✓ T2** · for the same model → **elicit the prior on the marginal SD** (τ^{-1/2}[Q⁻¹]ⱼⱼ^{1/2}) and *[→ entry](../recs/spatial-areal/T2.md)*
...
**✗ T3** · for choosing a **prior scale in a scale-mixture / structured spatial model** → a *[→ entry](../recs/spatial-areal/T3.md)*
...
**✗ R1** · for an **ICAR / BYM / intrinsic-smoothing-spline** component (Q rank-deficient by design) → *[→ entry](../recs/spatial-areal/R1.md)*
...
**✗ R2** · for a **disconnected ICAR graph** (islands / multiple components) → applying a **single *[→ entry](../recs/spatial-areal/R2.md)*
...
**✗ Z1** · for **non-identified additive spatial effects** (ICAR/BYM/CAR effects, ANOVA codings, *[→ entry](../recs/spatial-areal/Z1.md)*
...
**✓ Z2** · for the same effects → the **N-corrected soft constraint sum(x) ~ normal(0, s·N)** works *[→ entry](../recs/spatial-areal/Z2.md)*
(converges).
...
**✓ Z3** · for the same effects → a **hard sum-to-zero** via an exact linear map from N−1 iid raw *[→ entry](../recs/spatial-areal/Z3.md)*
...
**✗ Z4** · for **choosing soft vs hard** sum-to-zero → assuming one is **universally faster** does *[→ entry](../recs/spatial-areal/Z4.md)*
**NOT** work.
...
**✓ Z5** · for a **missing areal unit** (no data in a region) → let the hierarchical / CAR structure *[→ entry](../recs/spatial-areal/Z5.md)*
...
**✗ K1** · for representing a **squared-exponential (RBF) GP inside the SPDE/FEM** (sparse-precision / *[→ entry](../recs/spatial-areal/K1.md)*
...
**✓ K2** · for wanting a **fast RBF-like GP** → use an **inducing-point sparse GP or random Fourier *[→ entry](../recs/spatial-areal/K2.md)*
...
**✗ A1** · for a **regression/GLM on spatially-indexed data where a covariate is spatially *[→ entry](../recs/spatial-areal/A1.md)*
...
**✓ G1** · for a **border discontinuity / artifact in a fitted spatial map** → **interpret it before *[→ entry](../recs/spatial-areal/G1.md)*
...
**✗ N1** · for a **spatial GP whose covariance is numerically evaluated only on X_train** → *[→ entry](../recs/spatial-areal/N1.md)*
...
**✗ N2** · for a **spatial GP kernel with a sqrt distance on the diagonal** (i==j) → autodiff through *[→ entry](../recs/spatial-areal/N2.md)*
...
**✓ N3** · for a **GP held-out / cross-validation claim** → require an **explicit held-out validation *[→ entry](../recs/spatial-areal/N3.md)*
...
**✗ V1** · for a **spatial model with a GP/covariance term plus an independent noise term** → assuming *[→ entry](../recs/spatial-areal/V1.md)*
...
**✗ I1** · for an **ICAR Poisson areal (disease-mapping) model** → treating the ICAR nodes **without *[→ entry](../recs/spatial-areal/I1.md)*
...
**✗ D1** · for regressing a **pairwise dissimilarity response with spatial priors on additive site *[→ entry](../recs/spatial-areal/D1.md)*
...
**✗ M1** · for **optimizing a constrained-parameter spatial model** → conflating what **MLE vs MAP** *[→ entry](../recs/spatial-areal/M1.md)*
...
**✗ B1** · for **BKMR** (recognized as GP regression with an ARD kernel) with a **variable-selection *[→ entry](../recs/spatial-areal/B1.md)*
...
**✗ X1** · for **validating a spatial sampler with SBC** → trusting a **single scalar uniformity *[→ entry](../recs/spatial-areal/X1.md)*
...
**✗ C1** · for **divergences caused by a hard clip on a constrained parameter** → keeping the flat *[→ entry](../recs/spatial-areal/C1.md)*
